...
from src.signal_sweep.handlers.text_handler import TextHandler, _parse_text
from src.signal_sweep.shared.source import Source
from src.signal_sweep.shared.signal_stream import StreamData
...
async def test_text_handler(
    ...
    expected_results = [StreamData(ip, source.url) for ip, source in product(expected_ips, sample_sources)]
    ...
    with patch('asyncio.get_event_loop') as mock_get_event_loop:
        ...
        for source in sample_sources:
            ...
            assert all([actual_result in expected_results for actual_result in actual_results])
